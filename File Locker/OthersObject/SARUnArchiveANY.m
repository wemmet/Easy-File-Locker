...
- (void)zipDecompress{
    ...
    //_destinationPath = [_destinationPath stringByAppendingPathComponent:tmpDirname];
    NSError *error = nil;
    //BOOL unzipped = [SSZipArchive unzipFileAtPath:_filePath toDestination:_destinationPath overwrite:YES password:nil error:&error delegate:self];
    BOOL unzipped = [SSZipArchive unzipFileAtPath:_filePath
                    toDestination:_destinationPath
                        overwrite:YES
                         password:nil
                  progressHandler:^(NSString *entry, unz_file_info zipInfo, long entryNumber, long total) {
        NSLog(@"解压进度：%f",entryNumber/(total * 1.0));
...
- (void)decompress7z{
    ...
    [[[Decoder7zHelper alloc] init] extractLZMAFileAtPath:_filePath outPath:_destinationPath];
#endif
}
#else
- (void)decompress7z{
